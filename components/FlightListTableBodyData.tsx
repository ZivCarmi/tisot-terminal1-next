...
const FlightListTableBodyData = () => {
  ...
  return flights.map((flight) => {
    const checkInData =
      flight.CHCKZN && flight.CHCINT && flight.CHCKZN + flight.CHCINT;
    ...
    return (
      <tr key={flight._id}>
        ...
        <td className="p-2 whitespace-nowrap text-gray-700 bg-blue-50">
          <strong>{formatFlightTime(flight.CHPTOL)}</strong>
          <div>{formatFlightDate(flight.CHPTOL)}</div>
        </td>
        {!isArrivals && (
          <td className="p-2 whitespace-nowrap text-gray-700">
            <span>{checkInData || t("notSpecified")}</span>
          </td>
        )}
